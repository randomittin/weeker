"""The ``atlas verify`` command group (016 §T-14/T-17/T-24/T-35).
...
* ``atlas verify schema``   — schema introspection assertion (``verify.schema``).
* ``atlas verify ingest``   — the M1 ingestion gate (``--stage parse`` for S1 only).
* ``atlas verify concepts`` — the M2 concept gate.
* ``atlas verify bank``     — the M3 bank gate (``--caselets`` folds in the caselet bar).
* ``atlas verify adaptive`` — the M4 synthetic-replay gate (no DB required).
* ``atlas verify all``      — every gate in order, rendered as a PASS/FAIL/SKIP table.

Like ``atlas doctor``, each gate reports PASS, SKIP (prerequisite absent — DB
...
from atlas.verify import schema as schema_verify
...
def _render_table(results: list[GateResult]) -> int:
    table = Table(title="atlas verify all")
...
def register(group_app: typer.Typer) -> None:
    """Attach the milestone gates to the ``atlas verify`` group."""
