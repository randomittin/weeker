...
Wires the ``atlas`` console entrypoint and registers the six command groups
...
``atlas.<pkg>.cli`` module exposing ``register(app: typer.Typer)`` — no edit to
this file required. ``atlas doctor`` is the M0 gate.
"""
...
from atlas.cli.doctor import render, run_checks

app = typer.Typer(help="Atlas — adaptive NISM exam-prep learning engine.", no_args_is_help=True)
...
_GROUPS: dict[str, tuple[typer.Typer, str]] = {
    "ingest": (ingest_app, "atlas.ingest.cli"),
    "generate": (generate_app, "atlas.generate.cli"),
    "study": (study_app, "atlas.learn.study_cli"),
    "mock": (mock_app, "atlas.learn.mock_cli"),
    "status": (status_app, "atlas.learn.status_cli"),
    "verify": (verify_app, "atlas.verify.cli"),
}
