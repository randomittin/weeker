"""Study & flashcard TUI and the ``atlas study`` command group (015 §3, T-34/T-35).
...
* :class:`StudyApp` — one question per screen; ``a``–``d`` picks an answer, then
  ...
  attempt through :func:`atlas.learn.attempt_service.record_attempt`; the
...
``register`` attaches ``atlas study run`` (a live adaptive session), ``atlas
study verify`` (the synthetic M4 gate), ``atlas study replay`` (mastery
consistency), and ``atlas study flash``. Because ``cli/main.py`` marks the study
group ``no_args_is_help`` (and must not be edited), bare ``atlas study`` prints
the command list; ``atlas study run`` starts a session.
"""
...
from atlas.learn import retention, scheduler
...
def register(group_app: typer.Typer) -> None:
    """Attach the study/flash commands to the ``atlas study`` group."""
