...
is graded until submit. This module is the **pure engine** the ``atlas mock``
TUIs wrap:
...
* :func:`compose_standalone_items` — a blueprint-stratified 90-question standalone
  section, reusing the committed :mod:`atlas.learn.scheduler` mock composer;
* :func:`start_mock` / :func:`save_progress` / :func:`resume_mock` — a
  :class:`~atlas.core.models.MockExam` row whose ``config`` jsonb persists the item
...
from atlas.learn import predict, scheduler
...
def item_from_question(q: Question, section: str = STANDALONE) -> MockItem:
    """Build a :class:`MockItem` from an ORM :class:`~atlas.core.models.Question`."""
    return MockItem(
        question_id=q.id,
