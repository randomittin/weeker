"""The ``atlas status`` command group (016 §T-41/T-50).
...
* ``atlas status show`` — the learner dashboard (T-41): heatmap, due counts,
  flags, prediction band, tonight's refill plan and (from day 3) the realized
  calibration table;
* ``atlas status refill`` — the nightly loop (T-50). Spec names this
  ``atlas refill``; ``cli/main.py`` is frozen with no ``refill`` group, so it is
  wired here as ``atlas status refill`` (documented deviation, same pattern as
  ``atlas mock diagnose``). It runs generate-refill → bank verify → status print
  in one command.
"""
...
from atlas.core.models import Course
from atlas.learn import refill, status
...
def register(group_app: typer.Typer) -> None:
    """Attach the status/refill commands to the ``atlas status`` group."""
    ...
    def refill_cmd(
        ...
        """Nightly loop: generate --refill + bank verify + status print (T-50; spec: `atlas refill`)."""
        with session_scope() as db:
            c = _resolve_course(db, course)
            if c is None:
                typer.echo("No course found — ingest a course first.")
