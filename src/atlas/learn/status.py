"""``atlas status`` — the learner dashboard (016 §T-41, 017 §6).
...
* **prediction band** — whole-bank expected score / CI / ``P(pass)`` (suppressed
  when coverage is too thin), from :func:`atlas.learn.predict.predict_from_db`;
* **tonight's refill plan** — the weak-concept refill quotas from
  :func:`atlas.generate.targets.compute_targets`;
...
from atlas.core.models import Chapter, Concept, Course, Flashcard, Mastery
from atlas.generate.targets import compute_targets
from atlas.learn import elo, predict, retention
...
@dataclass
class StatusReport:
    """Everything ``atlas status`` prints for one learner/course."""
