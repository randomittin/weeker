"""The ``weeker ingest`` command group (016 §T-10..T-17).

``register`` attaches the real ingestion commands to the group created in
``cli/main.py`` (which loads ``weeker.ingest.cli`` and calls ``register`` — no
edit to that file needed):

* ``weeker ingest run`` — the full S0-S6 pipeline: manifest → parse → structure →
  chunk → embeddings → concepts → objectives + blueprint.
* ``weeker ingest review`` — launch the S5 human review TUI.
* ``weeker ingest verify-parse`` — the S1 parse gate per source.
* ``weeker ingest verify`` — the full M1 ingestion gate.
* ``weeker ingest verify-concepts`` — the M2 concept gate.
...
from weeker.ingest.chunk import chunk_course
from weeker.ingest.concepts import run_concepts
from weeker.ingest.embeddings import build_hnsw_index, embed_chunks
from weeker.ingest.manifest import load_manifest, upsert
from weeker.ingest.objectives import run_objectives
from weeker.ingest.parse import parse_source, persist_pages
from weeker.ingest.review import ReviewApp
from weeker.ingest.structure import build_structure, persist_structure
...
def register(group_app: typer.Typer) -> None:
    """Attach the ingestion commands to the ``weeker ingest`` group."""
    ...
    def run(
        ...
        typer.echo("ingestion complete — run 'weeker ingest review' for the S5 human pass.")
