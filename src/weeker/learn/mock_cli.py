"""The ``weeker mock`` command group + its Textual exam UIs (016 §T-36/T-40/T-42).
...
* ``weeker mock run`` — a timed standalone mock (T-40): one question per screen,
...
* ``weeker mock resume`` — resume the latest paused mock exactly where it stopped;
* ``weeker mock full`` — the full-pattern mock (T-42): 90 standalone + 6 one-mark
  + 3 two-mark caselets, with the per-section prediction + calibration + skip
  policy strategy report printed on the summary;
* ``weeker mock diagnose`` — the Day-0 diagnostic (T-36). Spec names this
  ``weeker diagnose``; ``cli/main.py`` is frozen with no ``diagnose`` group, so it
  is wired here as ``weeker mock diagnose`` (documented deviation, same pattern as
  ``weeker study flash``).
"""
...
from weeker.learn import diagnose, full, mock
...
def _echo_mock_outcome(app: MockApp) -> None:
    if app.paused:
        typer.echo("Mock paused — resume with `weeker mock resume`.")
        return
    r = app.result
    if r is None:
...
def register(group_app: typer.Typer) -> None:
    """Attach the mock/diagnostic commands to the ``weeker mock`` group."""
    ...
    def diagnose_cmd(
        ...
        """Run the Day-0 diagnostic and seed initial mastery (T-36; spec: `weeker diagnose`)."""
        with session_scope() as db:
            c = _resolve_course(db, course)
            if c is None:
                typer.echo("No course found — ingest a course first.")
