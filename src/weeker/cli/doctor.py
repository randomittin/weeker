"""``weeker doctor`` — the M0 readiness gate.
...
from weeker.core.llm import complete
...
def check_model_keys() -> CheckResult:
    ...
    if not config.llm_key_present():
        return CheckResult("model_keys", "SKIP", "WEEKER_LLM_API_KEY not set")
...
def check_embed_dim() -> CheckResult:
    if not config.embed_key_present():
        return CheckResult("embed_dim", "SKIP", "WEEKER_EMBED_API_KEY not set")
    try:
...
def render(results: list[CheckResult]) -> int:
    table = Table(title="weeker doctor")
