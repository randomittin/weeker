...
src/atlas/
  ...
  cli/       typer app; `atlas doctor` + command groups
...
|---|---|---|
| `DATABASE_URL` | `sqlite:///./atlas.db` | Postgres+pgvector in production; SQLite locally/tests |
| `ATLAS_MODEL_GENERATOR` / `_VERIFIER_CHEAP` / `_SOLVER_BLIND` / `ATLAS_MODEL_EMBED` | `…` | Provider model ids (G3 solver must be a different family) |
| `ATLAS_EMBED_DIM` | `1536` | Embedding dimension; sets `courses.embedding_dim` and the `vector(dim)` columns |
| `ATLAS_LLM_API_KEY` / `ATLAS_EMBED_API_KEY` | _(empty)_ | API keys; absent ⇒ live checks SKIP |
| `ATLAS_LLM_BASE_URL` / `ATLAS_EMBED_BASE_URL` | OpenAI-compatible | Chat + embeddings endpoints |
...
DATABASE_URL=sqlite:///./atlas.db .venv/bin/alembic upgrade head
.venv/bin/python -m atlas.verify.schema   # asserts all tables + one_correct_per_question
.venv/bin/atlas doctor                 # M0 readiness gate (exit 0)
```
