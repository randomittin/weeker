...
| T-14 | `ingest/embeddings.py` S4 — chunk embeddings→pgvector (JSON on sqlite), HNSW build guarded to Postgres, per-chapter cosine smoke retrieval; `verify/ingest_checks.py` full = **M1 gate** | `pytest -q tests/test_ingest_embeddings.py`; `atlas ingest verify` (M1) | PASS |
