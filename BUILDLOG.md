...
| T-03 | `core/config.py` — every 016 §T-03 constant verbatim; `MODEL_*`/`EMBED_*` from env with placeholder defaults; single source of tunables | `python -c "from weeker.core import config"` + `tests/test_config_is_single_source.py` (warn-only grep lint) | PASS |
| T-02 | Full schema (21 tables) `core/models.py` (SQLAlchemy 2.0); `core/db.py` engine/session; `core/types.py` cross-dialect `Vector`/JSONB/arrays; alembic `0001` full DDL (pgvector guarded to Postgres); `verify/schema.py` | `alembic upgrade head && python -m weeker.verify.schema` (SQLite) + `tests/test_models_sqlite.py`, `tests/test_schema_verify.py` | PASS (21 tables + `one_correct_per_question`) |
...
| T-05 | `cli/main.py` typer app + 6 sub-app groups (ingest/generate/study/mock/status/verify) with wave-attach hook; `cli/doctor.py` (`weeker doctor`) db/migrations/keys/live/embed-dim checks, SKIP when absent | `weeker doctor` exit 0 — **M0 gate** | PASS (all live checks SKIP, exit 0) |

# BUILDLOG — Wave 1a (Ingestion pipeline)

| Task | What | Gate | Result |
|---|---|---|---|
| T-10 | `ingest/manifest.py` S0 — pydantic manifest.yaml models (course + sources + 017 §1 exam block + blueprint overrides), loader, course/source upsert by sha256 | `pytest -q tests/test_ingest_manifest.py` (valid load; missing file / bad blueprint sum / dup sha rejected) | PASS |
| T-11 | `ingest/parse.py` S1 — PyMuPDF blocks/spans extraction, de-hyphenation, header/footer strip (HEADER_FOOTER_PAGE_SHARE), table serialization, OCR fallback behind OCR_MIN_CHARS | `pytest -q tests/test_ingest_parse.py` (synthetic fitz PDFs; de-hyphenation + header detection; coverage≥0.98, no silent-empty pages) | PASS |
| T-12 | `ingest/structure.py` S2 — strategy chain outline→font-heuristic→LLM (`prompts/structure.txt`); chapters non-overlapping, ≥95% page coverage, depth≤2 | `pytest -q tests/test_ingest_structure.py` (fixture PDFs per strategy; structure gate as code) | PASS |
| T-13 | `ingest/chunk.py` S3 — section-bounded chunking, atomic tables/examples, content_hash, overlap per config | `pytest -q tests/test_ingest_chunk.py` (token histogram in [CHUNK_MIN,MAX]; coverage≥0.97; property test: no chunk crosses chapter boundary) | PASS |
| T-14 | `ingest/embeddings.py` S4 — chunk embeddings→pgvector (JSON on sqlite), HNSW build guarded to Postgres, per-chapter cosine smoke retrieval; `verify/ingest_checks.py` full = **M1 gate** | `pytest -q tests/test_ingest_embeddings.py`; `weeker ingest verify` (M1) | PASS |
| T-15 | `ingest/concepts.py` S5 map+reduce — chapter-batched extraction (`prompts/concept_extract.txt`), embedding merge at CONCEPT_MERGE_COS, prereq resolution + DAG cycle check, concept→chunk retrieval (topK≥MIN_COS); leaves review_status='pending' | `pytest -q tests/test_ingest_concepts.py` (merge near-dupes; DAG cycle detection; e2e leaves only 'pending') | PASS |
...
| —   | `ingest/cli.py` `register(group_app)` → `weeker ingest run` (S0-S6 orchestration: manifest→parse→structure→chunk→embed→concepts→objectives+blueprint), `review` (launch TUI), `verify-parse`, `verify` (M1), `verify-concepts` (M2); real functions, no stub | `weeker ingest --help` lists 5 commands; imports resolve | PASS |

# BUILDLOG — Wave 2 (Adaptive learn loop)

| Task | What | Gate | Result |
|---|---|---|---|
| T-30 | `learn/elo.py` confidence-weighted Elo (`update`/`mastery`/`expected`, K decays ELO_K_BASE→MIN with attempts, CONF_MULT) + `learn/retention.py` FSRS-lite (`retrievability`/`stability_update`/`days_until_due`, STAB_* floors) — pure functions, no I/O | `pytest -q tests/test_learn_elo.py tests/test_learn_retention.py` (mastery∈[0,1]; theta bounded; conf-wrong \|Δ\|>guess-wrong; stability monotone 3 successes; K decays) | PASS |
| T-31 | `learn/attempt_service.py` `record_attempt` transactional — Attempt row (theta_before/after), Mastery upsert, q_rating+pull_count update, misconception flow 1-3 (flag on wrong+sure, tag, flashcard upsert), analytics event | `pytest -q tests/test_learn_attempt.py` (one call mutates exactly expected rows; misconception fires only wrong+sure; flag clears after 2 confident-correct) | PASS |
| T-32 | `learn/scheduler.py` priority formula (PRIORITY_W weak/overdue/misconception/novelty), softmax sampling (SOFTMAX_T), desirable-difficulty pick, RESERVE_DAYS, variant exclusion, SESSION_MIX; mock composer blueprint-stratified + difficulty ramp + adjacency | `pytest -q tests/test_learn_scheduler.py` (500-session ρ>0.8 serve-share vs priority; zero variant pairs; mock per-chapter ±1) | PASS |
| T-33 | `learn/predict.py` expected score + bootstrap CI (PREDICT_BOOTSTRAP) + P(pass) + negative marking + coverage suppression (<PREDICT_MIN_COVERAGE) | `pytest -q tests/test_learn_predict.py` (uniform-0.8 learner predicted within ±8%) | PASS |
| T-34 | `learn/study_cli.py` — `StudyApp` (one Q/screen, a-d answer → 1/2/3 confidence → grade via `record_attempt`, explanation block, `p` page-ranged source peek, end-of-session Δθ report) + `FlashApp` (space-flip drain, 1/2/3 grade reschedules via `retention.stability_update`); `register` attaches `weeker study run/flash/verify/replay` | `pytest -q tests/test_learn_study.py` (Textual Pilot 5-Q session asserts 5 Attempt rows) | PASS (4 tests) |
| T-35 | `learn/verify_adaptive.py` synthetic 200-attempt Elo replay (Pearson r≥0.7 vs true ability, bounds + [0,1] held) + `learn/replay.py` mastery-recompute diff vs stored `Mastery` rows; `verify_adaptive()`/`replay()` exit 0 = **M4 gate** | `pytest -q tests/test_learn_verify.py`; `weeker study verify` exit 0 (r=0.863) | PASS (5 tests) |
...
| —  | `learn/mock_cli.py` `register`→`weeker mock run`(T-40)/`resume`/`full`(T-42)/`diagnose`(T-36) + `MockApp`/`DiagnosticApp` Textual UIs (no-feedback-until-submit, esc-pause, silent measurement); `learn/status_cli.py` `register`→`weeker status show`(T-41)/`refill`(T-50) | `pytest tests/test_mock_tui.py` (Pilot: mock answer→submit→review writes MockExam+Attempts; diagnostic collect→seed writes session) + CLI smoke (empty DB degrades gracefully) | PASS (2) |

| —  | `verify/cli.py` `register(group_app)`→ top-level `weeker verify` group: `schema` (introspection assert), `ingest [--stage parse]` (M1/S1), `concepts` (M2), `bank [--caselets]` (M3, prints full `BankReport`, exit 1 if not passed), `adaptive` (M4, DB-free), `all` (every gate in order → rich PASS/FAIL/SKIP summary table, exit nonzero on any hard-fail); wires real gate fns, no stub. Degrades to SKIP (not crash) on unreachable/empty/unmigrated DB, mirroring `weeker doctor` | `pytest -q tests/test_verify_cli.py` (12 tests: CliRunner per command vs empty sqlite → exit 0/SKIP; `verify all` renders summary table); `weeker verify all` bare env → table, exit 0 (adaptive PASS r=0.863, rest SKIP) | PASS (12) |

**M5 gate** = M4 green + T-40 + T-41 green → PASS. Full suite: **195 passed** (183 prior + 12 verify-CLI), ruff clean.

Wave-3 notes:
- CLI-path deviations (main.py frozen; no `diagnose`/`refill` groups): spec `weeker diagnose` → **`weeker mock diagnose`**; spec `weeker refill` → **`weeker status refill`** (same pattern as `weeker study flash`; documented in module docstrings + `--help`).
...
- Ambiguity resolved: 016 T-40 gate says "135-answer" (pre-017 full count) but standalone `weeker mock run` defaults to 90; the shared marks-weighted engine scores any item list, so the 135-answer gate runs against a 135-item standalone set and T-42 adds the 2-mark caselet negatives.
